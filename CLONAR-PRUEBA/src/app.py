...
def login():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if user:
                session['user_id'] = user['ID_usuario'] if rol == 'empleado' else user['cc']
                if rol == 'empleado':
...
def dashboard():
    ...
    cursor.execute('SELECT Nombre_producto, stock FROM producto ORDER BY stock DESC LIMIT 5')
    ...
    fecha_especifica = '2024-07-10'  # Ajusta a la fecha deseada
    cursor.execute('''
...
def ventas():
    ...
    if request.method == 'POST':
        if 'id_producto' in request.form and 'cantidad' in request.form:
            ...
            flash('Error en los datos del formulario')
...
def obtener_productos_filtrados(proveedor_id):
    cursor = db.db.cursor(dictionary=True)
    if proveedor_id:
        cursor.execute('SELECT * FROM producto WHERE ID_proveedor = %s', (proveedor_id,))
    else:
        cursor.execute('SELECT * FROM producto')
    productos = cursor.fetchall()
    cursor.close()
...
def calcular_total_compra(carrito):
    total = 0
    for item in carrito:
        total += item['cantidad'] * item['valor_producto']
    return total
...
@app.route('/compras/quitar-producto/<int:index>', methods=['POST'])
def quitar_producto(index):
    if 'carrito' in session:
        if 0 <= index < len(session['carrito']):
            ...
            flash('Índice de producto no válido', 'danger')

    return redirect(url_for('compras'))


@app.route('/compras/vaciar-carrito', methods=['POST'])
def vaciar_carrito():
...
def finalizar_compra():
    if 'carrito' in session and session['carrito']:
        try:
            ...
            # Crear nueva compra
            cursor.execute('INSERT INTO compras (fecha_compra, ID_usuario) VALUES (NOW(), %s)', (session['id_usuario'],))
            db.db.commit()
            compra_id = cursor.lastrowid

            # Insertar detalles de compra
            for item in session['carrito']:
                cursor.execute('INSERT INTO detalle_compras (ID_compra, ID_producto, cantidad, valor_compra_producto) VALUES (%s, %s, %s, %s)',
                               (compra_id, item['id_producto'], item['cantidad'], item['valor_producto']))
                db.db.commit()

                # Actualizar stock del producto
                cursor.execute('SELECT * FROM producto WHERE ID_producto = %s', (item['id_producto'],))
                producto = cursor.fetchone()

                if producto:
                    nuevo_stock = producto['stock'] + item['cantidad']
                    cursor.execute('UPDATE producto SET stock = %s WHERE ID_producto = %s', (nuevo_stock, item['id_producto']))
                    db.db.commit()

            session.pop('carrito', None)
            flash('Compra realizada correctamente', 'success')
        except mysql.connector.Error as err:
            flash(f'Error al realizar la compra: {err}', 'danger')
